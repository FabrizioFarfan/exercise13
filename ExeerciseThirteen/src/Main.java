...
public class Main {
    ...
    private static void exercise2() {
        ...
        for(int itemMax : items){
            if(itemMax > maxNumber){
                maxNumber = itemMax;
            }
        };
        System.out.println("Thee largest number is: " + maxNumber);

        for(int itemMin : items){
            if(itemMin < minNumber){
                minNumber = itemMin;
            }
        };
    ...
    private static void exercise5() {
        for (int timesTableNumber = 1; timesTableNumber <= 10; timesTableNumber++) {
            for(int number = timesTableNumber; number <= timesTableNumber*10 ; number += timesTableNumber){
